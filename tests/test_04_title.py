...
class Test04TitleAPI:
    ...
    def test_01_title_not_auth(self, client):
        ...
        assert response.status_code == 200, (
            'Проверьте, что при GET запросе `/api/v1/reviews/` без токена авторизации возвращается статус 200'
        )
    ...
    def test_02_title_admin(self, admin_client):
        ...
        assert response.status_code == 400, (
            'Проверьте, что при POST запросе `/api/v1/reviews/` с не правильными данными возвращает статус 400'
        )
        ...
        assert response.status_code == 201, (
            'Проверьте, что при POST запросе `/api/v1/reviews/` с правильными данными возвращает статус 201'
        )
        ...
        assert type(response.json().get('id')) == int, (
            'Проверьте, что при POST запросе `/api/v1/reviews/` возвращаете данные созданного объекта. '
            'Значение `id` нет или не является целым числом.'
        )
        response = admin_client.get('/api/v1/reviews/')
        assert response.status_code == 200, (
            'Проверьте, что при GET запросе `/api/v1/reviews/` возвращает статус 200'
        )
        data = response.json()
        ...
        response = admin_client.get(f'/api/v1/reviews/?genre={genres[1]["slug"]}')
        data = response.json()
        assert len(data['results']) == 2, (
            'Проверьте, что при GET запросе `/api/v1/reviews/` фильтуется по `genre` параметру `slug` жанра'
        )
        response = admin_client.get(f'/api/v1/reviews/?category={categories[0]["slug"]}')
        data = response.json()
        assert len(data['results']) == 1, (
            'Проверьте, что при GET запросе `/api/v1/reviews/` фильтуется по `category` параметру `slug` категории'
        )
        response = admin_client.get('/api/v1/reviews/?year=2000')
        data = response.json()
        assert len(data['results']) == 1, (
            'Проверьте, что при GET запросе `/api/v1/reviews/` фильтуется по `year` параметру года'
        )
        response = admin_client.get('/api/v1/reviews/?name=Поворот')
        data = response.json()
        assert len(data['results']) == 2, (
            'Проверьте, что при GET запросе `/api/v1/reviews/` фильтуется по `name` параметру названия'
        )
        invalid_data = {
            'name': 'Поворот', 'year': 'дветыщи шесть', 'genre': [genres[1]['slug']],
            'category': categories[1]['slug'], 'description': 'Крутое пике'
        }
        response = admin_client.post('/api/v1/reviews/', data=invalid_data)
        code = 400
        assert response.status_code == code, (
            'Проверьте, что при POST запросе `/api/v1/reviews/`, поле year валидируется '
    ...
    def test_03_titles_detail(self, client, admin_client):
        ...
        assert response.status_code != 404, (
            'Страница `/api/v1/reviews/{title_id}/` не найдена, проверьте этот адрес в *urls.py*'
        )
        assert response.status_code == 200, (
        ...
        assert type(data.get('id')) == int, (
            'Проверьте, что при GET запросе `/api/v1/reviews/{title_id}/` возвращаете данные объекта. '
            'Значение `id` нет или не является целым числом.'
        )
        ...
        response = admin_client.patch(f'/api/v1/reviews/{titles[0]["id"]}/', data=data)
        assert response.status_code == 200, (
            'Проверьте, что при PATCH запросе `/api/v1/reviews/{title_id}/` возвращается статус 200'
        )
        data = response.json()
        assert data.get('name') == 'Новое название', (
            'Проверьте, что при PATCH запросе `/api/v1/reviews/{title_id}/` возвращаете данные объекта. '
        ...
        assert data.get('category') == categories[1], (
            'Проверьте, что при PATCH запросе `/api/v1/reviews/{title_id}/` изменяете значение `category`.'
        )
        assert data.get('name') == 'Новое название', (
            'Проверьте, что при PATCH запросе `/api/v1/reviews/{title_id}/` изменяете значение `name`.'
        )

        response = admin_client.delete(f'/api/v1/reviews/{titles[0]["id"]}/')
        assert response.status_code == 204, (
            'Проверьте, что при DELETE запросе `/api/v1/reviews/{title_id}/` возвращаете статус 204'
        )
        response = admin_client.get('/api/v1/reviews/')
        test_data = response.json()['results']
        assert len(test_data) == len(titles) - 1, (
            'Проверьте, что при DELETE запросе `/api/v1/reviews/{title_id}/` удаляете объект'
        )
    ...
    def check_permissions(self, user, user_name, titles, categories, genres):
        ...
        response = client_user.patch(f'/api/v1/reviews/{titles[0]["id"]}/', data=data)
        assert response.status_code == 403, (
            f'Проверьте, что при PATCH запросе `/api/v1/reviews/{{title_id}}/` '
            f'с токеном авторизации {user_name} возвращается статус 403'
        )
        response = client_user.delete(f'/api/v1/reviews/{titles[0]["id"]}/')
        assert response.status_code == 403, (
            f'Проверьте, что при DELETE запросе `/api/v1/reviews/{{title_id}}/` '
            f'с токеном авторизации {user_name} возвращается статус 403'
        )
    ...
    def test_04_titles_check_permission(self, client, admin_client):
        ...
        assert response.status_code == 401, (
            'Проверьте, что при POST запросе `/api/v1/reviews/` '
            'без токена авторизации возвращается статус 401'
        )
        response = client.patch(f'/api/v1/reviews/{titles[0]["id"]}/', data=data)
        assert response.status_code == 401, (
            'Проверьте, что при PATCH запросе `/api/v1/reviews/{{title_id}}/` '
            'без токена авторизации возвращается статус 401'
        )
        response = client.delete(f'/api/v1/reviews/{titles[0]["id"]}/')
        assert response.status_code == 401, (
            'Проверьте, что при DELETE запросе `/api/v1/reviews/{{title_id}}/` '
            'без токена авторизации возвращается статус 401'
        )
